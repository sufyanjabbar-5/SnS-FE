...
const HomeHero: React.FC<HomeHeroProps> = ({ slides }) => {
  ...
  return (
    <section className="relative w-full h-[650px] overflow-hidden bg-cover bg-center">
      {slides.map((slide, index) => (
        ...
        >
          ...
          <div className={`absolute right-0 w-[904px] h-[904px] pointer-events-none ${slide.circleContainerClassName || 'top-1/2 -translate-y-1/2'}`}>
            <div className="relative w-full h-full flex items-center justify-center">
              ...
              <div className={`relative z-10 ${slide.imageClassName || ''} mobile-screen`}>
          ...
          {/* Content */}
          <div className="relative max-w-screen-2xl mx-auto px-6 md:px-20 h-full flex flex-col justify-center">
            <div className="max-w-[725px]">
              <div className="flex items-center gap-3 mb-8">
                {slide.badges.map((badge, i) => (
                  <div key={i} className="bg-[#c2ddff] border border-[rgba(194,221,255,0.25)] rounded-full px-[13px] py-1 flex items-center gap-2 shadow-sm">
              ...
              <h1 className="text-white text-5xl font-semibold leading-[61px] mb-4 mobile-section-title">
                {slide.title}
              </h1>

              <p className="text-white text-base font-medium leading-6 mb-8 max-w-[632px] mobile-section-text">
                {slide.description}
              </p>

              {slide.checkList && (
                <div className={`grid ${slide.checkList.length > 3 ? 'grid-cols-2' : 'grid-cols-1'} gap-4 mb-8 max-w-[650px]`}>
      ...
      {/* Carousel Dots */}
      <div className="absolute bottom-[94.5px] right-[56px] flex items-center gap-3 z-20">
        {slides.map((_, index) => (
          <button
            key={index}
